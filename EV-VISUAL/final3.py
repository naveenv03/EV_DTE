...
def basic():
        if request.method =='POST':
            ...
            City_Roads = (request.form['City_Roads'])
            ...
            encoded_driving_style = (request.form['encoded_drivin)g_style'])
            encoded_tire_type = (request.form['encoded_tire_type'])
            y_pred = [[quantity,City_Roads ,Motor_way , Country_roads,consumption ,ac , park_heating, avg_speed,ecr_deviation, encoded_driving_style,encoded_tire_type]]
            prediction_outcome = model.predict(y_pred)
            return render_template('G:\EV\Templates\Forms.html', p=prediction_outcome)
